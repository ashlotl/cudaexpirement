...
int main() {
  ...
  double * newtemp;
  double * newmass;
  cudaMallocManaged(&x,numSamples*sizeof(double));
  cudaMallocManaged(&y,numSamples*sizeof(double));
  cudaMallocManaged(&z,numSamples*sizeof(double));
  cudaMallocManaged(&tx,numSamples*sizeof(double));
  cudaMallocManaged(&ty,numSamples*sizeof(double));
  cudaMallocManaged(&tz,numSamples*sizeof(double));
  cudaMallocManaged(&temp,numSamples*sizeof(double));
  cudaMallocManaged(&mass,numSamples*sizeof(double));
  cudaMallocManaged(&newtx,numSamples*sizeof(double));
  cudaMallocManaged(&newty,numSamples*sizeof(double));
  cudaMallocManaged(&newtz,numSamples*sizeof(double));
  cudaMallocManaged(&newtemp,numSamples*sizeof(double));
  cudaMallocManaged(&newmass,numSamples*sizeof(double));
  printf("Filling...\n");
  //Fill values:
  int xi=0;
  int yi=0;
  ...
  while(n>0) {
    ...
    osmate<<<numSamples,1>>>(TEMP_RATE,MASS_RATE,TEMP_PUSH,REPULSION_RATE,numSamples,ALTITUDE,GRAV_CONST,x,y,z,tx,ty,tz,newtx,newty,newtz,mass,temp,newmass,newtemp);
    cudaDeviceSynchronize();
    // printf("Joy to the world, the gpu has finished\n");
    // printf("Synchronized.\n");
